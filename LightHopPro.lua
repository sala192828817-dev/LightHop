--[[
    LIGHT HOP PRO — ETAPA 4 (POLIMENTO FINAL)
    UI (Etapa 1) + lógica real (Etapa 2) + Auto Hop (Etapa 3), com correções
    de bugs, performance, responsividade e consistência visual.

    CORREÇÕES DESTA ETAPA:
    - [Perf/Visual] Cada card virou CanvasGroup: 1 tween de fade (GroupTransparency)
      em vez de ~9 tweens por card (fundo + cada texto/botão individualmente).
      Isso também corrigia uma inconsistência visual: o fundo aparecia antes
      do texto, dando um "flash" de caixa vazia.
    - [BUG CRÍTICO] Ajuste acima trocou o card de Frame para CanvasGroup, mas
      CanvasGroup NÃO é subclasse de Frame no Roblox — o filtro de limpeza da
      lista (`child:IsA("Frame")`) deixaria de reconhecer os cards antigos e
      eles se acumulariam a cada Fetch/Refresh. Corrigido para checar pelo
      nome ("Card_..."), robusto independente da classe da instância.
    - [BUG] Reabrir o painel pelo toggle flutuante usava um tamanho salvo em
      atributos (Offset). No mobile o painel usa Scale, então esse Offset
      salvo era sempre 0 — reabrir no celular deixava o painel com 0x0. Agora
      recalcula o tamanho certo via applyResponsiveLayout() na hora de abrir.
    - [Responsividade] Barra de filtros e linha de modo do Auto Hop viraram
      ScrollingFrame horizontal (barra invisível) para nunca estourar/cortar
      em telas estreitas, em vez de vazar para fora do card.
    - [Alinhamento] CountLabel ganhou TextTruncate para não sobrepor a barra
      de progresso central quando o texto de status é mais longo.
    - [Limpeza] Removido código morto (pingDot oculto e pingColor não usado,
      já cobertos pela cor no texto de Status).
    - NotifyHolder ganhou AutomaticSize para os toasts terem bounds corretos.

    Servidores reais via API pública do Roblox; ping/FPS vêm da própria API;
    Região/Bandeira aparecem sempre no card (ver nota abaixo — a API pública
    não expõe datacenter, então o valor honesto é "Desconhecida 🏳️" até que
    uma fonte real de região seja plugada).
]]
...
local autoHopRunning = false -- trava síncrona: evita 2ª rotina mesmo antes do task.spawn retornar
local MAX_CONSECUTIVE_FAILURES = 5

function AutoHop.Start()
    -- Prevenção de múltiplas rotinas: a trava é setada de forma síncrona,
    -- antes do task.spawn, porque a variável de thread só fica disponível
    -- depois que a corrotina atinge seu primeiro yield.
    if autoHopRunning then return end
    autoHopRunning = true
    State:Set("autoHopOn", true)
    State:Set("autoHopStatusText", nil)
    Notify.Show("Auto Hop ativado.", "success", 2)

    autoHopThread = task.spawn(function()
        local consecutiveFailures = 0

        while State:Get("autoHopOn") do
            -- Integração com os filtros: garante que o pool de servidores
            -- usado pelo Auto Hop esteja alinhado ao modo selecionado
            -- (Melhor Ping / Mais Vazios), buscando de novo se necessário.
            local mode = State:Get("autoHopMode")
            if not State:Get("loading") and (#State:Get("servers") == 0 or State:Get("sort") ~= mode) then
                State:Set("autoHopStatusText", "🔎 Buscando servidores...")
                State:Set("sort", mode)
                ServerManager.Fetch()
                while State:Get("loading") and State:Get("autoHopOn") do
                    task.wait(0.2)
                end
            end

            if not State:Get("autoHopOn") then break end

            local best = ServerManager.BestServer(mode)

            if best then
                State:Set("autoHopStatusText", ("⚡ Conectando: %s (%dms)"):format(string.sub(best.id, 1, 8), best.ping))
                local ok, err = pcall(joinServer, best)
                if ok then
                    consecutiveFailures = 0
                else
                    consecutiveFailures += 1
                    warn("[LightHopPro] Falha ao entrar no servidor: " .. tostring(err))
                    State:Set("autoHopStatusText", "⚠ Falha ao entrar no servidor")
                end
            else
                consecutiveFailures += 1
                State:Set("servers", {})
                State:Set("autoHopStatusText", "⚠ Nenhum servidor elegível")
            end

            -- Tratamento de falhas: após muitas tentativas seguidas sem sucesso,
            -- desliga o Auto Hop sozinho em vez de girar para sempre em silêncio.
            if consecutiveFailures >= MAX_CONSECUTIVE_FAILURES then
                Notify.Show("Auto Hop desativado: muitas falhas seguidas.", "error", 4)
                State:Set("autoHopOn", false)
                break
            end

            -- Cooldown entre tentativas (interrompível se o usuário desligar antes)
            local waited = 0
...
function AutoHop.Stop()
    if not autoHopRunning and not State:Get("autoHopOn") then return end
    State:Set("autoHopOn", false)
    State:Set("autoHopStatusText", nil)
    autoHopRunning = false
    ...
    Notify.Show("Auto Hop desativado.", "info", 2)
end

-- Cleanup: garante que o loop nunca sobreviva ao painel/personagem
...
local applyResponsiveLayout -- definida na seção RESPONSIVIDADE; usada também pelo toggle flutuante
...
FilterBar.AutomaticCanvasSize = Enum.AutomaticCanvasSize.X
...
-- Forward declarations: permitem que a Filter Bar (definida antes, mais acima
-- no arquivo) sincronize com os botões de Modo do Auto Hop (definidos mais
-- abaixo, no rodapé) sem duplicar lógica.
local modeButtons, styleModeButton, autoHopMode, refreshActiveLabel
...
local function createFilterButton(key, text, widthGrow)
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        -- Integração com o Auto Hop: "Menor Ping"/"Mais Vazios" no filtro
        -- também é um modo válido do Auto Hop, então mantemos os dois em sincronia.
        if (key == "ping" or key == "empty") and modeButtons and styleModeButton then
            autoHopMode = key
            State:Set("autoHopMode", key)
            for k, b in pairs(modeButtons) do styleModeButton(b, k == autoHopMode) end
            if refreshActiveLabel then refreshActiveLabel() end
        end
    end)
...
local function createServerCard(server)
    ...
    -- entrada suave do card (um único tween via CanvasGroup, em vez de
    -- tweenar cada TextLabel/TextButton individualmente)
    task.delay(0.02 * server.rank, function()
        if card and card.Parent then
            Util.Tween(card, { GroupTransparency = 0 }, 0.18)
        end
    end)
...
local function renderServerList(servers)
    for _, child in ipairs(ListFrame:GetChildren()) do
        if child ~= EmptyLabel and child.Name:sub(1, 5) == "Card_" then
            child:Destroy()
        end
    end
...
modeButtons = {}
autoHopMode = "ping"
local ModeHolder = Instance.new("ScrollingFrame")
...
ModeHolder.ScrollingDirection = Enum.ScrollingDirection.X
ModeHolder.ScrollBarThickness = 0
ModeHolder.CanvasSize = UDim2.new(0, 0, 0, 0)
ModeHolder.AutomaticCanvasSize = Enum.AutomaticCanvasSize.X
...
function styleModeButton(btn, active)
    if active then
        btn.BackgroundColor3 = Theme.AccentDim
        btn.TextColor3 = Theme.Accent
...
local function createModeButton(key, text)
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        -- Integração com os filtros: o modo do Auto Hop também é uma opção
        -- de ordenação válida na Filter Bar, então mantemos as duas em sincronia.
        if not State:Get("loading") and State:Get("sort") ~= key then
            activeFilter = key
            for k2, b2 in pairs(filterButtons) do
                styleFilterButton(b2.frame, k2 == activeFilter)
            end
            State:Set("sort", key)
            ServerManager.Fetch()
        end
...
local function refreshAutoHopVisual()
    ...
    if on then refreshActiveLabel() else ActiveLabel.Text = "Auto Hop desativado" end
end

-- Feedback visual em tempo real (buscando / conectando / falha) enquanto o
-- Auto Hop está ligado; some sozinho e volta ao texto padrão quando limpo.
State:OnChange("autoHopStatusText", function(text)
    if not State:Get("autoHopOn") then return end
    if text then
...
-- Os switches já nascem "ligados" (mesmo visual da referência). Para o
-- visual nunca mentir sobre o estado real, isso efetivamente liga o Auto
-- Hop de verdade no boot; se preferir iniciar desligado, troque o valor
-- inicial passado para makeSwitch(...) mais acima para "false".
if AutoHopSwitch.Get() then
    AutoHop.Start()
else
    refreshAutoHopVisual()
end
...
do
    ...
    ToggleBtn.MouseButton1Click:Connect(function()
        ...
        if open then
            Main.Visible = true
            applyResponsiveLayout() -- recalcula o tamanho correto para o viewport atual (desktop ou mobile)
            local target = Main.Size
            Main.Size = UDim2.new(0, 0, 0, 0)
            Util.Tween(Main, { Size = target }, 0.22, Enum.EasingStyle.Back)
        else
